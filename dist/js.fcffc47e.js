...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  '../js/generateCard.js': [function (require, module, exports) {
    ...
    const _ElementClass = require('./Element.class.js')
    ...
    function generateCard (param) {
      ...
      for (let i = 0; i < param.length; i++) {
        const article = new _ElementClass.Element('1', 'article', 'card').elt
        const anchor = new _ElementClass.Element('2', 'a', 'card__anchor').elt
        const divBg = new _ElementClass.Element('3', 'div', 'card__bg').elt
        const divRecipe = new _ElementClass.Element('4', 'div', 'card__recipe').elt
        const divTitle = new _ElementClass.Element('5', 'div', 'card__title').elt
        const title = new _ElementClass.Element('6', 'h3', 'card__title__h3').elt
        const divTime = new _ElementClass.Element('7', 'div', 'card__title__time').elt
        const iconTime = new _ElementClass.Element('8', 'i', 'card__title__clock').elt
        const time = new _ElementClass.Element('9', 'p', 'card__title__time__clock__number').elt
        const cardContent = new _ElementClass.Element('10', 'div', 'card__content').elt
        const ulIngredients = new _ElementClass.Element('11', 'ul', 'card__ingredients').elt
        const description = new _ElementClass.Element('12', 'p', 'card__description').elt // -------1 article----------------------------------------
    ...
    function displayIngredients (ingredients, ulIngredients) {
      const _iterator = _createForOfIteratorHelper(ingredients)
      let _step

      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          const ingredient = _step.value
          const liIngredient = new _ElementClass.Element('1', 'li', 'ingredientsList').elt
          const ingredientName = new _ElementClass.Element('2', 'p', 'ingredientsList__item').elt
          const quantity = new _ElementClass.Element('3', 'p', 'ingredientsList__quantity').elt // ------1 liIngredient-------------------------------------
          ...
          const unit = new _ElementClass.Element('4', 'p', 'ingredientsList__units').elt // ------4 units--------------------------------------------
  ...
  }, { './Element.class.js': '../js/Element.class.js' }],
  '../js/recipes.js': [function (require, module, exports) {
    'use strict'
  ...
  '../js/index.js': [function (require, module, exports) {
    ...
    const _generateCard = require('./generateCard.js')

    const _recipes = require('./recipes.js');

    (0, _generateCard.generateCard)(_recipes.recipes)
  }, { './generateCard.js': '../js/generateCard.js', './recipes.js': '../js/recipes.js' }],
  '../../node_modules/parcel-bundler/src/builtins/hmr-runtime.js': [function (require, module, exports) {
    ...
    if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
      ...
      const ws = new WebSocket(protocol + '://' + hostname + ':' + '63187' + '/')
